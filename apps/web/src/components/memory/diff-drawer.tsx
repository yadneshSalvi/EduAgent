 ...
 * DiffEditor (MonacoFileDiff, shared with the memory explorer), inline mode,
 * our diff token colors. Header, serif italic: "EduAgent updated its memory
 * of you."
 *
 * The exit animation is hand-rolled (no AnimatePresence): the shell slides
 * out from cached `shown` state while the Monaco pane unmounts the moment
 * `commit` goes null (QA F2). AnimatePresence would freeze the WHOLE subtree
 * — Monaco included — until the exit finished, so a route change landing
 * mid-exit disposed the DiffEditor during the transition and could leave a
 * renderer spinning. This way disposal always happens at close time, on a
 * stable page.
 */
...
export function DiffDrawer({ commit, onClose, headerLine }: DiffDrawerProps) {
  ...
  // `commit` opens/closes; `shown` is what the shell renders while sliding
  // out (cleared when the exit animation settles).
  const [shown, setShown] = useState<MemoryCommit | null>(commit);
  if (commit !== null && commit !== shown) setShown(commit);
  const open = commit !== null;

  const files = useMemo(() => (shown ? parseUnifiedDiff(shown.diff) : []), [shown]);
  const [activeIndex, setActiveIndex] = useState(0);
  useEffect(() => setActiveIndex(0), [shown?.sha]);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={
          reducedMotion ? { opacity: open ? 1 : 0 } : { y: open ? 0 : '100%' }
        }
      ...
      >
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[300px_1fr]">
          ...
          <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto border-b p-5 md:border-b-0 md:border-r">
            <p className="text-body font-medium leading-snug">{shown.headline}</p>
            {shown.bullets.length > 0 ? (
              <ul className="flex flex-col gap-1.5">
                {shown.bullets.map((bullet, index) => (
                  <li
                    key={index}
          ...
          <div className="flex min-h-0 min-w-0 flex-col">
            ...
            <div className="min-h-0 flex-1">
              {/* Monaco mounts strictly while open — see the header comment. */}
              {open && activeFile ? (
                <MonacoFileDiff
                  key={`${shown.sha}:${activeFile.path}`}
